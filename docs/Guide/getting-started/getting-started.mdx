...
To begin, require `@sapphire/framework` and create a new `SapphireClient`. This is where you can customize Sapphire's and discord.js' behaviour. The Sapphire client extends discord.js', so everything from `Client` is available in `SapphireClient`!
