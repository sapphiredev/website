...
Similar to what you learnt in [Creating Commands](../getting-started/creating-a-basic-command), we will create a `listeners` folder in the same directory as your `main` file. For this page, we will make a listener for the `ready` event.
...
:::caution
Unlike with commands, the method for running a listener is called `run`, not `messageRun`. This distinction is because `mesasgeRun` is our first step towards supporting Slash Command / Context Actions which need seperately named methods.
:::
