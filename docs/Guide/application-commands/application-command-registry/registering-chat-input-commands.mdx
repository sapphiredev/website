...
default logger provided by `Sapphire`. If this is the first time registering the command, the command ID is logged on
the [`info`][loglevel-info] level, which is the default. If you want to view the ID after the initial register please
make sure to [`Configure Log Level Guide`][configure-logging] to the [`debug`][loglevel-debug] level. If you have
configured the Sapphire Logger to only log errors or fatal messages then please check
[`Configuring Log Level Guide`][configure-logging] to learn how to change it to either [`info`][loglevel-info] or
