...
export class UserPrecondition extends AllFlowsPrecondition {
  ...
  private async doBanlistCheck(guildId: string | null) {
    // If the command is ran in a DM, return ok.
    if (guildId === null) return this.ok();

    // Do a database query to check if the guild is banned. In this code snippet we use Prisma, but you can use any database library you want. We also use Result.fromAsync in combination with findFirstOrThrow to make checking if the guild was in the banlist or not easier.
