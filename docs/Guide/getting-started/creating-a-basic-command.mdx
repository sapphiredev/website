...
Note that by default Sapphire does not load message commands. You will need to construct your `SapphireClient` with
the `loadMessageCommandListeners` property set to true. See the `SapphireClientOptions` docs.
[Client Options](https://www.sapphirejs.dev/docs/Documentation/api-framework/interfaces/SapphireClientOptions#loadmessagecommandlisteners)
