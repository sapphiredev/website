...
[`Application Command Registry`][acr] means to retrieve an already existing one for the command name you specify or generating a brand new one. This may sound more complicated than
it actually is, so bare with us here. Acquiring an [`Application Command Registry`][acr] can be done in two ways:

- By calling the `ApplicationCommandRegistries.acquire` method
- By using the [`registerApplicationCommands`][register-application-commands] method of the [`Command`][command] class (which receives a pre-defined registry for the command it's present in)
...
This means that this registry will only concern itself about the current `ping` command - it's not going to impact any other commands.
else.
