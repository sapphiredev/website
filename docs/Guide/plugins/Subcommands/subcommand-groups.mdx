...
```typescript ts2esm2cjs|{8-19,23-27}|{9-20,24-28}
...
```typescript ts2esm2cjs|{8-19}|{9-20}
import { Subcommand } from '@sapphire/plugin-subcommands';
import type { Args } from '@sapphire/framework';
...
export class UserCommand extends Subcommand {
  public constructor(context: Subcommand.Context, options: Subcommand.Options) {
    super(context, {
      ...
      name: 'vip',
      subcommands: [
        { name: 'list', messageRun: 'messageList', chatInputRun: 'chatInputList', default: true },
        {
          name: 'action',
          type: 'group',
          entries: [
...
[getting-started-message-command-usage]: ./getting-started.mdx#message-command-usage
