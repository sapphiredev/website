...
Note that by default Sapphire does not load the listeners required for processing message commands. You will need to construct your `SapphireClient` with
the `loadMessageCommandListeners` option set to `true`. See more information about the [client options](https://www.sapphirejs.dev/docs/Documentation/api-framework/interfaces/SapphireClientOptions#loadmessagecommandlisteners).
