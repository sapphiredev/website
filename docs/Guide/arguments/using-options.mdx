...
Imperative optionsare options you specify in the command options, and using them means Sapphire will *only* parse the specified options.

We recommend this method because it can avoid unexpected situations where something, for example in [`args.rest('string')`][rest], is parsed as an option mistakenly.
