...
```bash npm2yarn2pnpm
npm install @sapphire/plugin-subcommands @sapphire/framework @sapphire/utilities discord.js@13.x
```

## Message Command Usage

```typescript ts2esm2cjs|{8-23,27-31}|{9-24,28-32}
...
`messageRemove`. You can map the value of `messageRun` to the name of any method in the class you want, but we recommend
the naming convention above to keep things organized.
...
```typescript ts2esm2cjs|{9-26}|{10-27}
import { Subcommand } from '@sapphire/plugin-subcommands';
import type { Args } from '@sapphire/framework';
...
export class UserCommand extends Subcommand {
  ...
  // Register Application Commands below...
}
```

Keep in mind that that `default` is only functional for Message Command Subcommands as mentioned in the previous
section.
