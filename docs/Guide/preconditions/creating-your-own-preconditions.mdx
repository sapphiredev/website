...
Clicking on [`run`][preconditionrun] above will take you to a method instead called `messageRun`. This is because this documentation reflects the latest commit to the `main` branch of `@sapphire/framework` which is has a breaking change included that changed the name of the method. For the purposes of this guide however, you can use `run` as long as you are using the latest NPM published version of `@sapphire/framework`.
