...
[`cooldownDelay`][cooldowndelay] property in their options. This represents the time in milliseconds a user will have to
wait after using a command to use it again. Here's a basic example:
...
to `1` by default. For example, a `cooldownDelay` of 10 seconds, and a `cooldownLimit` of 2 will effectively mean that
you'd be able to use the command twice every 10 seconds.
