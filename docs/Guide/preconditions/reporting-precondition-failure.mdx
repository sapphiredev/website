...
Clicking on [`CommandDeniedPayload`][payload] above will take you to an interface instead called `MessageCommandDeniedPayload`. This is because this documentation reflects the latest commit to the `main` branch of `@sapphire/framework` which is has a breaking change included that changed the name of the interface. For the purposes of this guide however, you can use `CommandDeniedPayload` as long as you are using the latest NPM published version of `@sapphire/framework`.
