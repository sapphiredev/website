...
`Cron` class can be used to work with the cron jobs time. It can be used to get the next time a cron job should be
executed. By default it uses the current `UTC` time to compare. However, you can pass a `Time` object to compare with
too in `next` method.
...
`TimerManager` class can be used to manage timers. It can be used to create a timer that will be executed every `X` ms
and also to set timeout.
