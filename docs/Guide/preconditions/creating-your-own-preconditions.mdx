...
Preconditions are classes that will determine whether or not a command should be blocked according to certain conditions.
Sapphire has many built in preconditions ready for use that are outlined in the coming pages, such as for
...
as `AdminOnly` _or_ both `OwnerOnly` and
