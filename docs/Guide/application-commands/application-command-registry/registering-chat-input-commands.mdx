...
Register a Chat Input Command by calling the [`registerChatInputCommand`][rcic] method on the registry, this looks like:

```typescript ts2esm2cjs|{7,12-16}|{7,12-16}
...
We recommend using the [`SlashCommandBuilder`][djs-slash-builder-docs] here for input verification, but you are also
free to write the `discord.js` [`ApplicationCommandData`][djs-command-data-docs] JSON object yourself as the argument to
the method.
...
ID(s) to the `idHints` option, we can prevent new commands from accidentally being created.
