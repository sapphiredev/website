...
export class PingCommand extends Command {
  ...
  public async messageRun(message: Message) {
    await editLocalized(message, { keys: 'commands/ping:success_with_args', formatOptions: { latency: 5 } });
  }
}
```
...
export class PingCommand extends Command {
  ...
  public async messageRun(message: Message) {
    await replyLocalized(message, { keys: 'commands/ping:success_with_args', formatOptions: { latency: 5 } });
  }
}
```
