...
import type { TextBasedChannelTypes } from 'discord.js';
...
import { Command, Argument } from '@sapphire/framework';
import { isNullishOrEmpty } from '@sapphire/utilities';

export class Command {
  public async messageRun(message: Message, args: Args) {
    const parameter = args.finished ? null : await args.pick(Command.numberOrString);
    if (!parameter) return message.reply('Please provide a number or a string');
    return message.reply('Your parameter is a valid number or a string!');
  }
