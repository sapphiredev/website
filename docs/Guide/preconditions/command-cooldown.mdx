...
1. `cooldownDelay`: This option allows you to set the cooldown time that the command will have. For example, if you set
   ...
   seconds, and if the user executes the command while it is in cooldown time then the `commandError` event will be
   fired. You can see the Listeners guide to see how to use these events.
